...
class NonMovingStock(models.TransientModel):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(NonMovingStock, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                          submenu=submenu)
        if not ast.literal_eval(self.env['ir.config_parameter'].sudo().get_param('flexibite_com_advance.groups_ids')):
            if view_type == 'form':
    ...
    def print_report(self):
        ...
        if self.report_type == 'pdf':
            if self.env.context.get('send_mail'):
                ...
                vals_new = {'products': final_non_moving_data}
                return self.env.ref('flexibite_com_advance.non_moving_stock_report').report_action(self,
                                                                                                   data=vals_new)
