...
class ProductProduct(models.Model):
    ...
    #     print ("\n\n calll====",self,write_date,fields,con)
    ...
    @api.model
    def load_latest_product(self, write_date, fields, context):
        config_id = self.env['pos.config'].browse([context.get('location')])
        context.update({'location': config_id.picking_type_id.default_location_src_id.id})
        variant_ids = self.with_context(context).search(
            [('write_date', '>', write_date), ("sale_ok", "=", True), ("available_in_pos", "=", True)])
        tmpl_ids = [x.product_tmpl_id.id for x in variant_ids]
        tmpl_ids = list(dict.fromkeys(tmpl_ids))
    ...
    def expiry_product_alert(self):
        ...
        if notification_days:
            ...
            email_list = []
            template_id = self.env.ref('flexibite_com_advance.email_template_product_expiry_alert')
    ...
    def calculate_product(self, config_id):
        ...
        if pos_session and pos_session.store_id:
            ...
            if setting and setting.group_multi_company and not setting.company_share_product:
                product_ids = self.with_context({'location': config.picking_type_id.default_location_src_id.id}).search(
