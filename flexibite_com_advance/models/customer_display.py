...
class customer_display(models.Model):
    ...
    @api.model
    def load_customer_display_data(self, config_id, company_id):
        customer_data = {'currency': False, 'customer_display_data': False, 'load_config': False}
            ...
            if config_obj and config_obj.customer_display_details_ids:
                ...
                if cust_disp_ids:
                    customer_data.update({
                        'customer_display_data': self.search_read([('id', 'in', cust_disp_ids.ids)]) or False})

                load_config = config_obj.search_read([('id', '=', config_id)],
                                                     ['customer_display', 'image_interval',
                                                      'customer_display_details_ids',
                                                      'enable_customer_rating',
                                                      'set_customer'])

                if load_config:
                    customer_data.update({'load_config': load_config})

        if company_id:
            company_obj = self.env['res.company'].browse(company_id)
            if company_obj and company_obj.currency_id:
                customer_data.update({'currency': company_obj.currency_id.read()})
