...
class pos_cache(models.Model):
    ...
    def refresh_cache(self):
        Product = self.env['product.product'].with_user(self.compute_user_id.id)

        # products = Product.search(self.get_product_domain())
        products = Product.with_context({'location': self.config_id.picking_type_id.default_location_src_id.id}).search(
