...
class stock_production_lot(models.Model):
    ...
    @api.model
    def search_serial_lot_data(self, prod_id):
        return self.sudo().search_read(
            [('product_id', '=', prod_id)], fields=['remaining_qty', 'id', 'product_id', 'name', 'ref', 'life_date'],
            order='life_date ASC')
                ...
                if life_date < today_date:
                    ...
                    if each_stock_lot.alert_date:
                        ...
                        if alert_date <= today_date:
                            ...
                            each_stock_lot.expiry_state = 'is_ok'
                            each_stock_lot.write({'state_check': 'is_ok'})
            else:
                each_stock_lot.write({'state_check': ''})
        else:
            for each_stock_lot in self:
                each_stock_lot.write({'state_check': 'is_ok', 'expiry_state': 'is_ok'})
