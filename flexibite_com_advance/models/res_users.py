...
class ResUsers(models.Model):
    ...
    def check_location_id(self):
        for store_id in self.store_ids:
            if store_id.company_id not in self.company_ids:
                raise Warning(_("Select Shops according to user's company!"))

    pos_security_pin = fields.Char(size=20, string="Security PIN")
    ...
    default_store_id = fields.Many2one('pos.store', string="Default Store")
    store_ids = fields.Many2many("pos.store", string='Allow Store')
    ...
    pos_user_type = fields.Selection(
        [('cashier', 'Cashier'), ('delivery_user', 'Delivery User'), ('cook', 'Cook'), ],
        string="POS User Type", default='cashier')  # ('salesman', 'Sales Person')
    ...
    access_create_purchase_order = fields.Boolean("Create Purchase Order")
    rfid_no = fields.Char('RFID No.')
    change_sales_person = fields.Boolean("Change Sales Person")
    ...
    pos_category_ids = fields.Many2many('pos.category', 'pos_categ_user_rel', 'categ_id', 'user_id',
