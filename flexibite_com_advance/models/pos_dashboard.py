...
class posSessions(models.Model):
    ...
    def payment_by_journal_pie_chart(self, company_id):
        ...
        SQL = ""
        if len(pos_ids) > 1:
            SQL = "pp.pos_order_id IN  %s" % str(tuple(pos_ids))
        else:
            if len(pos_ids) > 0:
                SQL = "pp.pos_order_id =  %s" % pos_ids[0]
        if len(pos_ids) > 0:
    ...
    def get_journal_line_chart_data(self, start_date, end_date, journal, company_id):
        ...
        sql_query = """SELECT
                         ...
                         extract(day from po.date_order AT TIME ZONE 'UTC' AT TIME ZONE '%s') AS time_duration,
                         ppm.name AS journal, ppm.id
                        FROM pos_payment AS pp
                         INNER JOIN pos_payment_method AS ppm ON ppm.id = pp.payment_method_id
                         INNER JOIN pos_order AS po ON po.id = pp.pos_order_id
                         WHERE po.date_order >= '%s'
                         AND po.date_order <= '%s' %s
                         AND po.company_id = %s
                         GROUP BY time_duration, journal, ppm.id ORDER BY extract(day from po.date_order AT TIME ZONE 'UTC' AT TIME ZONE '%s') ASC
...
class posOrder(models.Model):
    ...
    def customer_avg_spent_per_visit(self, start, end, company_id):
        ...
        current_time_zone = self.env.user.tz or 'UTC'
        sql_query = f"""SELECT 
                                ROUND(SUM(amount_paid)/COUNT(*),2) AS total_amount 
                            FROM 
                                pos_order 
                            WHERE 
                                company_id = {company_id} 
                            AND 
                                ((date_order AT TIME ZONE 'UTC' AT TIME ZONE '{current_time_zone}')::date >= (current_date AT TIME ZONE 'UTC' AT TIME ZONE '{current_time_zone}')::date - 30)"""
        self._cr.execute(sql_query)
        cust_data = self._cr.dictfetchall()
        if cust_data:
            customer_avg_spent = cust_data[0].get('total_amount', 0)
        else:
            customer_avg_spent = 0
        return {'cust_avg_spent_per_visit': customer_avg_spent,
    ...
    def customer_avg_visit(self, company_id):
        ...
        current_time_zone = self.env.user.tz or 'UTC'
        sql_query = f"""SELECT 
                                ROUND(COUNT(partner_id)/30,2) AS customer 
                            FROM 
                                pos_order 
                            WHERE 
                                company_id = {company_id} 
