...
//	        	event.stopImmediatePropagation();
            ...
            this.keydown_uom = function(event){
                event.stopImmediatePropagation();
                self.keyboard_uom(event);
            };
        },
        start: function(){
            var self = this;
            ...
            if(self.pos.config.change_product_uom){
                $(document).keydown(_.bind(this.keydown_uom, self));
            }
        },
        keyboard_uom: function(event){
            var self = this;
            ...
            if(self.gui.current_popup){
                ...
                if(event.keyCode == 13){
                    var res = self.pos.get_order().apply_uom();
                    if(res){
                        self.gui.current_popup.click_cancel();
                    } else{
                ...
                if(event.keyCode === $.ui.keyCode.UP){
                    var prev_el = $('.template_list.selected').prev();
                    if(prev_el.length > 0){
                        $('.template_list.selected').removeClass('selected');
                        $(prev_el).addClass('selected');
                        var uom_id = $('.template_list.selected').attr('id');
                        self.pos.get_order().set_custom_uom_id(uom_id);
                    }
                }
                if(event.keyCode === $.ui.keyCode.DOWN){
                    var next_el = $('.template_list.selected').next();
                    if(next_el.length > 0){
                        $('.template_list.selected').removeClass('selected');
        ...
        init: function(parent, options) {
            ...
            this.keyboard_shortcut = function(event){
                if(self.gui.get_current_screen() === "payment"){
                    var keytostring = event.key;
                    var selected_payment_method = _.find(self.pos.payment_methods, function(each_payment){
                        return each_payment.shortcut_key === keytostring;
                    })
                    if(selected_payment_method){
                        self.click_paymentmethods(selected_payment_method.id);
                    }
                }
                event.stopImmediatePropagation();
            };
