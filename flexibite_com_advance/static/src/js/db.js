...
//            Sale order extension
            ...
            this.products_purchase_data = false;
            this.bom_by_id = {};
            this.bom_line_ids = [];
            this.bom_by_id = {};            
        },
        add_prod_bom: function(prod_bom_data){
            var self = this;
            for(var i = 0, len = prod_bom_data.length; i < len; i++){
                var prod_bom_details = prod_bom_data[i];
                ...
                if(prod_bom_details && prod_bom_details.bom_line_ids){
                    _.each(prod_bom_details.bom_line_ids, function(each_bom_line_id) {
                        self.bom_line_ids.push(each_bom_line_id);
                    });
                }
            }
        },
        search_product_variant: function(query){
            try {
            ...
            for(var i = 0; i < this.limit; i++){
                var r = re.exec(utils.unaccent(this.product_search_string));
                if(r){
                    var id = Number(r[1]);
                    if(this.get_product_by_id(id).product_variant_count > 1 && this.get_product_by_id(id).is_primary_variant){
                        var product_template = this.template_by_id[this.get_product_by_id(id).product_tmpl_id]
                        results.push(product_template);
                    }
                }else{
                    break;
                }
        ...
        set_products_purchase_data: function (products_res) {
            this.products_purchase_data = products_res;
        },
        get_products_purchase_data: function () {
            return this.products_purchase_data;
        },
        get_product_variant: function(category_id){
            var product_ids  = this.product_by_category_id[category_id];
            var list = [];
            if (product_ids) {
                for (var i = 0, len = Math.min(product_ids.length, this.limit); i < len; i++) {
                    if(Number(this.product_by_id[product_ids[i]].product_variant_count) >= 2){
                        list.push(this.get_product_attribute_by_id(this.product_by_id[product_ids[i]].product_tmpl_id));
                    }else{
                        break
                    }
                }
        ...
        get_product_by_value_and_products: function(value_id, products){
            var list = [];
            for (var p=0; p<this.product_template_attribute.length;p++){
                for (var i = 0, len = products.length; i < len; i++) {
                    if(products[i].product_template_attribute_value_ids.indexOf(this.product_template_attribute[p].id) != -1){
                        if(this.product_template_attribute[p].product_attribute_value_id[0] === value_id){
        ...
        add_templates: function(templates){
            for(var i=0 ; i < templates.length; i++){
                ...
                for(var p=0; p < this.product_template_attribute.length; p++){
                    if (this.product_template_attribute[p].product_tmpl_id[0] === templates[i].id){
                        attribute_value_ids.push(this.product_template_attribute[p].product_attribute_value_id[0])
                    }
                }
