odoo.define('flexibite_com_advance.popup', function (require) {
                ...
                if(cashier && cashier.pos_security_pin){
                    if(cashier.pos_security_pin == old_pin){
                        if(new_pin == confirm_pin){
                            ...
                            ChangePinPromise.then(function(res){
                                if(res){
    ...
    var CreateProductPopupWidget = PopupWidget.extend({
        ...
        save_product_details: function() {
            ...
            if (self.uploaded_picture) {
                fields['image_1920'] = self.uploaded_picture.split(',')[1];
            }
            if (!fields.name) {
                ...
                fields['uom_id'] = Number(fields.uom_id);
                var params = {
                    model: 'product.template',
                    method: 'create_from_ui',
                    args: [fields],
                }
                var createProductPromise = new Promise(function(resolve, reject){
                ...
                createProductPromise.then(function(product_id){
                    var pro = Promise.resolve(self.pos.load_new_products())
    ...
    var ShowProductPopupWidget = PopupWidget.extend({
        ...
        save_product: function(){
            ...
            fields['uom_id'] = parseInt(fields.uom_id); 
            var EditProductPromise = new Promise(function(resolve, reject){
                var params = {
                    model: 'product.product',
                    method: 'write',
                    args: [[parseInt(self.product.id)], fields],
                }
            ...
            EditProductPromise.then(function(result){
                ...
                self.renderElement();
                $('.create-product-img').html('');
                $('.create-product-img').append("<img src='"+self.uploaded_picture+"'>");
                var all_products = _.uniq(self.pos.db.get_product_by_category(0), function(item, key, id) {
                                    return item.id;
                                });;
        ...
        init: function(parent, args) {
            ...
            this.keypress_order_number = function(e){
                if(e.which === 13){
                    ...
                    if(self.pos.config.enable_multi_store && self.pos.pos_session.store_id && self.pos.pos_session.store_id[0]){
                        domain = ['|',['store_id','=',self.pos.pos_session.store_id[0]],['store_id','=',false],['pos_reference','=', ret_o_ref_order]];
                    } else{
                        ...
                        if(res){
                            ...
                            if(self.pos.config.z_report){
                                ...
                                if(self.pos.config.iface_print_via_proxy){
                                    var report_name = "flexibite_com_advance.pos_z_thermal_report_template";
                                    var params = {
...
//	    		self.gui.show_screen('products');
    ...
    var RedeemLoyaltyPointsPopup = PopupWidget.extend({
        ...
        show: function(options){
            ...
            var LoyaltyRedeemPromise = new Promise(function(resolve, reject){
            ...
            LoyaltyRedeemPromise.then(function(partner){
                if(partner){
                    ...
                    if(amount_to_redeem <= (order.get_due() || order.get_total_with_tax())){
                        if(self.pos.config.loyalty_journal_id){
                            var loyalty_cashregister = _.find(self.pos.payment_methods, function(cashregister){
                                return cashregister.id === self.pos.config.loyalty_journal_id[0] ? cashregister : false;
                            });
    ...
    var CreateCardPopupWidget = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            if(!card_number){
                ...
                var ExistGiftCardPromise = new Promise(function(reject, resolve){
                    var params = {
                ...
                ExistGiftCardPromise.then(function(result){
            ...
            if(expire_date){
                if(checkbox_paid.checked){
                    ...
                    if(input_amount){
                        ...
                        var GiftCardPromise = new Promise(function(resolve, reject){
                            var params = {
                                model: "aspl.gift.card",
                                method: "create",
                                args: [{
                                    ...
                                    'expire_date':moment($('#text_expire_date').val()).format('YYYY-MM-DD'),
                        ...
                        GiftCardPromise.then(function(result){
                ...
                if(self.card_no){
                    var EditCardPromise = new Promise(function(resolve, reject){
                        var params = {
                            model: "aspl.gift.card",
                            method: "write",
                            args: [self.card_id,{'expire_date': moment(new_expire_date).format('YYYY-MM-DD')}]
                        }
                        rpc.query(params, {async: false}).then(function(result){
                            if(result){
                                resolve(result);
                    ...
                    EditCardPromise.then(function(result){
                        self.gui.close_popup();
        ...
        click_confirm: function(){
            ...
            if(self.card_no){
                ...
                if(!card_number){
                    ...
                    var ExchangeGiftCardPromise = new Promise(function(reject, resolve){
                        var params = {
                                model: 'aspl.gift.card',
                                method: 'search_read',
                    ...
                    ExchangeGiftCardPromise.then(function(result){
                ...
                if( exchange_card_no){
                    var ConfirmExchangeCardPromise = new Promise(function(resolve, reject){
                        var params = {
                            model: "aspl.gift.card",
                            method: "write",
                            args: [self.card_id,{'card_no':self.$('#new_card_no').val()}]
                        }
                        rpc.query(params, {async: false}).then(function(res){
                            if(res){
                                resolve(res);
                    ...
                    ConfirmExchangeCardPromise.then(function(res){
                        self.gui.close_popup();
                        self.pos.gui.chrome.screens.giftcardlistscreen.reloading_gift_cards();
                    });
                }
    ...
    var ConfirmationCardPayment = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            if(self.recharge_card){
                ...
                var partner = self.pos.db.get_partner_by_id(self.recharge_card.card_customer_id);
                if(partner){
                    order.set_client(partner);
                }
                order.set_recharge_giftcard(vals);
           ...
           /* window.document.body.removeEventListener('keypress',self.payment_self.keyboard_handler);
            ...
            $('#gift_voucher_text').keypress(function(e) {
                if (e.which == 13 && $(this).val()) {
                    ...
                    var GifVoucherPromise = new Promise(function(resolve, reject){
                        rpc.query(params, {async: false})
                        .then(function(res){
                            resolve(res);
                        })
                    ...
                    GifVoucherPromise.then(function(res){
                        if(res.length > 0){
                            var due = order.get_total_with_tax() - order.get_total_paid();
                            if (res && res[0].minimum_purchase <= total_pay && res[0].voucher_amount <= due){
            ...
            if (self.self_voucher){
                ...
                if (self.pos.config.gift_voucher_journal_id.length > 0){
                    for ( var i = 0; i < self.pos.payment_methods.length; i++ ) {
                        if (self.pos.payment_methods[i].id === self.pos.config.gift_voucher_journal_id[0]){
...
//	Cash In/Out Popup and Statement Popup
    ...
    var PrintCashInOutStatmentPopup = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            var GetCashInOutPromise = new Promise(function(resolve, reject){
                rpc.query(params, {async: false}).then(function(result){
                    if(result){
                        resolve(result);
                    }
            ...
            GetCashInOutPromise.then(function(result){
                var order = self.pos.get_order();
                if(user_id && result){
    ...
    var PutMoneyInPopup = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            var params = {
                ...
                args: [name, amount,session_id, 'money_in'],
            }
            rpc.query(params, {async: false}).then(function(result){
                if (result['error']) {
    ...
    var TakeMoneyOutPopup = PopupWidget.extend({
        ...
        click_confirm: function(){
            ...
            var params = {
                model: 'pos.session',
                method: 'take_money_in_out',
                args: [name, amount,session_id, 'money_out'],
            }
            rpc.query(params, {async: false}).then(function(result){
                if (result['error']) {
                    self.pos.db.notification('danger',result['error']);
...
//    Discard product popup
    var DiscardStockPickPopupWidget = PopupWidget.extend({
        template: 'DiscardStockPickPopupWidget',
        click_confirm: function(){
            var self = this;
            self.clear_cart();
            this.gui.close_popup();
    ...
    gui.define_popup({name:'discard_stock_pick', widget: DiscardStockPickPopupWidget});
                ...
                rpc.query(params, {async: false}).then(function(res){
                    if(res){
                        if(Object.keys(res['journal_details']).length == 0 && Object.keys(res['salesmen_details']).length == 0){
                            ...
                            if (self.pos.config.iface_print_via_proxy || (self.pos.config.other_devices && self.pos.config.epson_printer_ip)) {
                                var receipt = "";
                    ...
                    if(res){
                        ...
                        if (self.pos.config.iface_print_via_proxy || (self.pos.config.other_devices && self.pos.config.epson_printer_ip)) {
                            ...
                            for (var step = 0; step < no_of_copies; step++) {
                                receipt.push(QWeb.render('ProductSummaryReport', {
                                    widget: self,
                                    pos: self.pos,
                                    order: order,
                                    receipt: order.export_for_printing(),
                                ...
                                self.pos.print_receipt(receipt);
                            }
                        } else{
                            self.gui.show_screen('receipt');
                            }
...
//Order Summary Report
    var OrderSummaryPopupWidget = PopupWidget.extend({
                    ...
                    if(res){
                        if(Object.keys(res['category_report']).length == 0 && Object.keys(res['order_report']).length == 0 &&
                            ...
                            if(order.get_receipt()) {
                                ...
                                if(self.pos.config.iface_print_via_proxy || (self.pos.config.other_devices && self.pos.config.epson_printer_ip)){
                                    for (var i=0;i < num;i++) {
                                        receipt.push(QWeb.render('CustomOrderReceipt', {
                                            widget: self,
                                            pos: self.pos,
                                            order: order,
                                            receipt: order.export_for_printing(),
        ...
        print_token_receipt: function (receipt) {
            var self = this;
            var i = -1
            function ReceiptLoop(html) {
                setTimeout(function() {
                    if(html){
                        self.pos.proxy.printer.print_receipt(html);
                    }
                    i+=1;
                    if (i < receipt.length) {
...
//	Multi-Store Popup
    var MultiStorePopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            ...
            self.stock_location_data = options.stock_location_data;
            self._super(options);
                    ...
                    if(self.selected_id == self.pos.pos_session.store_id[0]){
                        ...
                        var UsersPromise = new Promise(function(resolve, reject){
                            rpc.query({
                                model: 'res.users',
                                method: 'write',
                                args: [self.pos.user.id,{'default_store_id':self.pos.store_rec.id}],
                            })
                        ...
                        UsersPromise.then(function(result){
                            if(result){
                                ...
                                var PosShopPromise = new Promise(function(resolve, reject){
                                    rpc.query({
                                        model: 'pos.store',
                                        method: 'write',
                                        args: [self.selected_id,{'store_manager':self.pos.user.id}],
                                    })
                                    .then(function(result) {
                                        if(result){
                                            resolve(result);
                                ...
                                PosShopPromise.then(function(result){
                                    if(result){
                                        self.pos.store_manager = self.pos.user.id;
                                    }
                                });
    ...
    var ReportPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            options = options || {};
            this._super(options);
            if(this.pos.config.iface_print_via_proxy  || (this.pos.config.epson_printer_ip && this.pos.config.other_devices)){
                this.enable_thermal_print = true;
            }else{
                this.enable_thermal_print = false;
            }
            this.renderElement();
            this.$el.find('.report_pdf.location, .report_thermal.location').click(_.bind(this.location_report, this));
            this.$el.find('.report_thermal.session, .report_pdf.session').click(_.bind(this.session_report, this));
            this.$el.find('.tablinks').click(_.bind(this.tablinks, this));
        },
        ...
        session_report: function(e){
            var self = this;
            var session_id = $(e.currentTarget).data('id');
            var pdf_or_thermal = $(e.currentTarget).attr('name');
            if(pdf_or_thermal === 'pdf'){
                new Promise(function(resolve, reject){
                    self.chrome.do_action('flexibite_com_advance.report_pos_inventory_session_pdf_front',{additional_context:{
                ...
                .then(function(report_html){
                    if(report_html && report_html[0]){
                        self.pos.proxy.printer.print_receipt(report_html[0])
                    }
                });
            }

        },
        location_report: function(e){
            var self = this;
            ...
            if(pdf_or_thermal === 'pdf'){
                ...
                let report_name = "flexibite_com_advance.front_inventory_location_thermal_report_template";
                var params = {
                    model: 'ir.actions.report',
                    method: 'get_html_report',
                ...
                .then(function(report_html){
                    if(report_html && report_html[0]){
                        self.pos.proxy.printer.print_receipt(report_html);
                    }
                });
            }
         },
...
//	        $("#loading").hide();
        ...
        renderElement: function() {
            var self = this;
            this._super();
            self.pos.db.get_supplier_list().then(function(supplier_list){
   ...
   /* var AddMoneyToCreditPopup = PopupWidget.extend({
    ...
    var DeliveryDetailPopupWidget = PopupWidget.extend({
        ...
        show: function(options){
            ...
            var order = this.pos.get_order();
            self.$('#delivery_detail_date').change(function(){
                order.set_delivery_date($(this).val())
            });
    ...
    var ProductPurchaseHistory = PopupWidget.extend({
        template: 'ProductPurchaseHistory',
        events: {
            'click i.history_cancel': 'click_cancel'
        },
        get_receipt_render_env: function() {
            var order = this.pos.get_order();
            var self = this;
            var po_history = self.pos.db.get_products_purchase_data();
            return {
                widget: this,
                pos: this.pos,
                order: order,
        ...
        show: function (options) {
            ...
            $(".print_function").click(function () {
                ...
                if(self.pos.proxy.printer){
                    if (order.get_last_order_print() == false) {
                        var report = QWeb.render('XmlReceiptProductsHistory', self.get_receipt_render_env());
                        self.pos.proxy.printer.print_receipt(report)
                    } else {
                        var report1 = QWeb.render('XmlReceiptLastOrder', self.get_receipt_render_env());
                        self.pos.proxy.printer.print_receipt(report1);
                    }
                }
                else{
                    self.pos.db.notification('danger', "Printer Offline!");
                    return;
                }
            }),
            $(document).ready(function () {
                $('.purchase_container').hide();
                $('.purchase_container:first').show();
                ...
                $('#tab1').click(function () {
                    var order = self.pos.get_order();
                    order.set_last_order_print(false);
                    $("#tab1").css("background-color", "#389466");
                    $("#cols1").css("background-color", "#389466");
                    $("#tab2").css("background-color", "#393939");
                    $("#cols2").css("background-color", "#393939");
                })
                $('#tab2').click(function () {
                    var order = self.pos.get_order();
                    order.set_last_order_print(true);
                    $("#tab2").css("background-color", "#389466");
                    $("#cols2").css("background-color", "#389466");
                    $("#tab1").css("background-color", "#393939");
                    $("#cols1").css("background-color", "#393939");
                })
            });
        },
    });
    gui.define_popup({ name: 'ProductPurchaseHistory', widget: ProductPurchaseHistory });

    var SingleProductPopup = PopupWidget.extend({
        template: 'SingleProductPopup',
        show: function (options) {
            self = this
            this._super(options)
            setTimeout(function () {
    ...
    gui.define_popup({ name: 'SingleProductPopup', widget: SingleProductPopup });
    ...
    var SetSalesPersonPopupWidget = PopupWidget.extend({
        template: 'SetSalesPersonPopupWidget',
        show: function(options){
            var self = this;
            options = options || {};
            this._super(options);
            this.salesperson_list = options.salesperson_list || [];
            this.is_selected = options.is_selected;
            this.renderElement();
            self.items;
            $(".sales-person-selection-item").on("click",function(e){
                $(".sales-person-selection-item").removeClass('selected');
                $(this).addClass('selected');
                var item = self.salesperson_list[parseInt($(e.target).data('item-index'))];
                self.items = item;
            });
        },
        click_confirm: function(){
            var self = this;
            var order = self.pos.get_order();
            var assign_sales_person = order.get_salesman_id();
            if(self.items){
                if(assign_sales_person && assign_sales_person == self.items.item.id){
                    order.set_salesman_id(self.items.item.id);
                } else {
                    order.set_salesman_id(self.items.item.id);
                }
            }
    ...
    gui.define_popup({name:'assign_sales_person_popup', widget: SetSalesPersonPopupWidget});

    var BomProductPopup = PopupWidget.extend({
        template: 'BomProductPopup',
        events: _.extend({}, PopupWidget.prototype.events, {
        ...
        show: function (options) {
            ...
            $('.bom_content').click(function(event){
                if($(this).hasClass('selected')){
                    $(this).removeClass('selected');
                    $(this).css('border','none');
                     $(this).attr("data-string", 'remove')
                }
                else{
                    $(this).addClass('selected');
                    $(this).attr("data-string", 'select')
                }
            });
        },
        click_confirm: function(){
            ...
            var line_ids = [];
            var updated_qty = [];
            $('.bom_content').each(function (index, value) {
                if($(this).attr('data-string') && $(this).attr('data-string') === 'remove' && $(this).attr('data-line-id')){
                    line_ids.push(Number($(this).attr('data-line-id')))
                }
            })
            _.each($('.js_quantity'), function(item){
                updated_qty.push({
                   'bom_line_qty':Number($(item).val()),
                   'bom_line_id':Number($(item).attr('name')),
                });
            });
            var order = this.pos.get_order();
            var selected_line = order.get_selected_orderline();
            var data = selected_line.get_bom_line_at_start();
            var update_order_line = _.each(data[0], function(item){
                _.each(updated_qty, function(qty){
                    if(qty.bom_line_id == item.id){
                        item['product_qty'] = qty.bom_line_qty;
                    }
                })
                if(_.contains(line_ids, Number(item.id))) {
                    item['select'] = false;
                } else {
                    item['select'] = true;
                }
           })
            selected_line.set_bom_line(update_order_line);
            self.gui.close_popup();
        },
        get_product_image_url: function(product_id){
            return window.location.origin + '/web/binary/image?model=product.product&field=image_medium&id='+product_id;
    ...
    gui.define_popup({ name: 'BomProductPopup', widget: BomProductPopup , });
...
//             this.combo_product_info = options.combo_product_info || false;
//             var combo_products_details = [];
//             var combo_products_details1 = [];
//             this.new_combo_products_details = [];
//             this.scroll_position = 0;
//             // console.log("this.product----->",this.product)
//             this.product.product_combo_ids.map(function(id){
//                 // console.log("self.pos.product_combo----->",self.pos.product_combo)
//                 // console.log("self.pos.product_combo------->",self.pos.product_combo_item)
//                 _.each(self.pos.product_combo_item,function(data){
//                     if(data.combo_id[0] == id){
//                         combo_products_details.push(data)
//                     }
//                 });

//                 // var record = _.find(self.pos.product_combo_item, function(data){
//                 //     return data.combo_id[0] === id;
//                 // });
...
//                     return data.id === id;
//                 });
//                 combo_products_details1.push(record);
//             });
//             _.each(combo_products_details1,function(each_combo_product){
//                 each_combo_product['product_ids'] = [];
//                 _.each(each_combo_product.product_combo_item_ids, function(product_ids){
//                    _.each(self.pos.product_combo_item,function(each_combo){
//                         if(each_combo.id == product_ids){
//                             each_combo_product['product_ids'].push(each_combo)
//                         }
//                     });
//                 });
//             });
//             // console.log("----->",combo_products_details1)
...
//             _.each(combo_products_details,function(each_combo_product){
//                _.each(self.pos.product_combo,function(each_combo){
//                     if(each_combo_product['combo_id'][0] == each_combo.id){
//                         each_combo_product['no_of_items'] = each_combo['no_of_items']
//                         each_combo_product['pos_category_id'] = each_combo['pos_category_id']
//                         each_combo_product['require'] = each_combo['require']
//                     }
//                 }); 
//             });
            
//             combo_products_details1.map(function(combo_line){
//                 // console.log("combo_line------>",combo_line)
//                 var details = [];
//                 if(combo_line.product_ids.length > 0){
//                     combo_line.product_ids.map(function(product_id){
//                         // console.log("combo_line------5663--->",combo_line)
//                         if(combo_line.require){
//                             var data = {
//                                 'no_of_items':combo_line.no_of_items,
//                                 'product_id':product_id,
...
//                                 'used_time':0
//                             }
//                             product_id['used_time'] = data['used_time']
//                             if(self.combo_product_info){
//                                 self.combo_product_info.map(function(line){
//                                     if(combo_line.id == line.id && line.product.id == product_id){
//                                         data['used_time'] =  line.qty;
//                                         product_id['used_time'] = line.qty
//                                     }
//                                 });
//                             }
//                             details.push(data);
...
//             // combo_products_details.map(function(combo_line){                
//             //     var details = [];
//             //     if(combo_line.product_id.length > 0){
//             //         console.log("combo_line----->",combo_line)
...
//             //             details.push(data);
//             //         }else{
//             //             // console.log("product_id--5648-->",product_id)
//             //             var data = {
//             //                 'no_of_items':combo_line.no_of_items,
//             //                 'product_id':combo_line['product_id'][0],
//             //                 'category_id':combo_line.pos_category_id[0] || false,
...
//             console.log("self.new_combo_products_details---->",self.new_combo_products_details)
//             this.renderElement();
//         },
//         collaps_div: function(event){
//             if($(event.currentTarget).hasClass('fix_products')){
...
//                             // if(combo_line.id == line_id && (_.contains(combo_line.product_ids, product_id) || (combo_line.pos_category_id[0] == category_id))){
// //                          if(combo_line.id == line_id && combo_line.pos_category_id[0] == category_id && (_.contains(combo_line.product_ids, product_id))){
//                             if(combo_line.id == line_id){
//                                 combo_line.product_ids.map(function(product_detail){

//                                     if(product_detail.product_id[0] == product_id){
//                                         product_detail.used_time = 0;
//                                     }
//                                 });
//                             }
...
//                                 if(product_detail.product_id[0] == product_id){
//                                     if(product_detail.no_of_items > product_detail.used_time && product_detail.no_of_items > added_item){                                        
//                                         // console.log("product_detail----->",product_detail.used)
//                                         product_detail.used_time += 1;
//                                         // console.log("product_detail.used_item---->",product_detail.used_time)
//                                     }
//                                 }
//                             });
...
//                             // });
//                             // combo_line.product_details.map(function(product_detail){
//                             //     if(product_detail.product_id.product_id[0] == product_id){
//                             //         alert("2")
//                             //         if(product_detail.no_of_items > product_detail.used_time && product_detail.no_of_items > added_item){                                        
//                             //             console.log("product_detail----->",product_detail.used)
//                             //             product_detail.used_time += 1;
...
//                 });
//             }
//             console.log("---5817-->",self.new_combo_products_details)
//             self.renderElement();
//         },
//         // select_product: function(event){
//         //     alert()
...
//         //         self.new_combo_products_details.map(function(combo_line){
//         //             if(!combo_line.require){                        
//         //                 if(combo_line.id == line_id && combo_line.pos_category_id[0] == category_id){
//         //                     var added_item = 0;
//         //                     console.log("combo_line---->",combo_line)
//         //                     combo_line.product_details.map(function(product_detail){
//         //                         added_item += product_detail.used_time;
//         //                     });
//         //                     // [combo_line].map(function(product_detail){
//         //                     //     console.log("product_detail----5782--->",product_detail)
//         //                     //     added_item += product_detail.used_time;
//         //                     // });
//         //                     [combo_line].map(function(product_detail){


//         //                         _.each(product_detail.product_details,function(each_product){
                                    
//         //                             if(each_product.product_id.product_id[0] == product_id){
//         //                                 console.log("each_product---->",each_product)
//         //                                 console.log("added_item---->",added_item)
//         //                                 if(each_product.no_of_items >= each_product.used_time && each_product.no_of_items > added_item){
//         //                                     each_product.used_time += 1;
//         //                                 }
//         //                             }
//         //                         });
...
//             self.new_combo_products_details.map(function(combo_line){
//                 if(combo_line.product_ids.length > 0){
//                     combo_line.product_ids.map(function(prod_detail){
//                     // console.log("----5882---->",prod_detail)
//                         if(prod_detail.require){
//                             // console.log("prod_detail--->",prod_detail)
//                             var product = self.pos.db.get_product_by_id(prod_detail.product_id[0]);
//                             if(product){
// //                              total_amount = self.product.get_price(pricelist, 1);
//                                 products_info.push({
//                                     "product":{
...
//             });
//             var selected_line = order.get_selected_orderline();
//             // console.log("products_info---->",products_info)
//             if(products_info.length > 0){
//                 if(selected_line){
// //                  selected_line.set_unit_price(total_amount);
//                     selected_line.set_combo_prod_info(products_info);
//                     var combo_price = 0;
//                     _.each(products_info,function(product_info){
//                         combo_price += product_info.price;
//                     }); 
//                     console.log("combo_price---->",combo_price)
//                     selected_line.set_unit_price(selected_line.get_unit_price() + combo_price);
//                 }else{
//                     alert("Selected line not found!");
//                 }
//             }else{
//                 console.log("selected_line.get_combo_prod_info()---->",selected_line.get_combo_prod_info())
//                 if(selected_line && !selected_line.get_combo_prod_info()){
//                     alert()
//                     order.remove_orderline(selected_line);
//                 }
...
//                 self.total_price = this.pos.get_order().get_selected_orderline().get_unit_price();
//             }
//             if(this.new_combo_products_details){
//                 _.each(this.new_combo_products_details,function(record){
//                     _.each(record.product_ids,function(product_detail){
//                         if(record.require){
//                             if(product_detail.price > 0){
//                                 self.total_price += product_detail.price
//                             }
//                         }else{
//                             if(product_detail.price > 0 && product_detail.used_time > 0){
//                                 self.total_price += product_detail.price
//                             }
//                         }
//                     });    
...
//             }
//             // this.total = 90;
//             console.log("self.new_combo_products_details--5911-->",this.new_combo_products_details)
//             this._super();
//             this.$el.find('.combo_header2_body').scrollTop(this.scroll_position);
//         },
//     });
